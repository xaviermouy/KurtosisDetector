...
class Detec:
    ...
    def save2Pamlab(self, outdir):
        ...
        annot.to_csv(os.path.join(outdir, str(self.output.fileName[0])) + str(self.output.fileExtension[0]) + ' chan' +  str(int(self.output.channel[0])) + ' annotations.log', sep='\t', encoding='utf-8', header=True, columns=cols, index=False)


    def save2Raven(self, outdir):
        cols = ['Selection', 'View', 'Channel', 'Begin Time (s)', 'End Time (s)', 'Low Freq (Hz)', 'High Freq (Hz)', 'Begin Path', 'File Offset (s)', 'Begin File', 'File Duration (s)', 'Species', 'Sound type', 'Detector', 'Distortion (DTW)', 'Kurtosis']
        annot = pd.DataFrame({'Selection': 0, 'View': 0, 'Channel': 0, 'Begin Time (s)': 0, 'End Time (s)': 0, 'Low Freq (Hz)': 0, 'High Freq (Hz)': 0, 'Begin Path': 0, 'File Offset (s)': 0, 'Begin File': 0, 'File Duration (s)': 0, 'Species': 0, 'Sound type': 0, 'Detector': 0, 'Distortion (DTW)': 0, 'Kurtosis': 0}, index=list(range(self.output.shape[0])))
        annot['Selection'] = range(1,self.output.shape[0]+1)
        annot['View'] = 'Spectrogram 1'
        annot['Channel'] = self.output['channel'] + 1
        annot['Channel'] = annot['Channel'].astype('int64')
        ...
        annot['Kurtosis'] = self.output['comment']
        annot.to_csv(os.path.join(outdir, str(self.output.fileName[0])) + str(self.output.fileExtension[0]) + '.chan' +  str(int(self.output.channel[0])) + '.Table.1.selections.txt', sep='\t', encoding='utf-8', header=True, columns=cols, index=False)
